...
EXAMPLE=system_files/usr/share/pi-core/pi-core.conf.example
cp "$EXAMPLE" "$WORK/pi-core.conf"
...
')
expect     "ssid is applied"            "would: add a wifi connection for SSID 'somenet'" "$OUT"
expect_not "the psk is not echoed back" 'hunter2hunter2' "$OUT"

head_ "every whitelisted key has a handler"
# The apply loop warns rather than dies if a key is listed with no handler, so
# without this a new key could be added to the table and silently do nothing.
OUT=$(run_conf 'PI_HOSTNAME=x
PI_SSH_KEY=ssh-ed25519 AAAA test@example
PI_TIMEZONE=UTC
PI_WIFI_SSID=n
PI_WIFI_PSK=p
PI_TAILSCALE_AUTHKEY=tskey-test
')
expect_not "no key without a handler" 'no handler for' "$OUT"
